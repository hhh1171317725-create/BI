...
- `/terminal`：无需本站登录即可使用的完整 SSH 网页终端。支持密码或私钥认证，凭据保存在服务器 `.runtime/ssh.env`，私钥保存在 `.runtime/ssh-private-key`，页面不会回显凭据。首次连接自动记录主机指纹到 `.runtime/ssh-known-hosts`，后续指纹变化时拒绝连接。公开终端会直接暴露服务器操作能力，只应在可信网络或额外的 Nginx 访问控制后使用。
- `/account`：账户设置。所有用户可修改自己的密码，管理员还可创建用户、重置密码以及停用或启用账号。
- `/account-vault`：管理员账户对应关系。每条记录保存一个关键词、多个账户 ID、多个投放国家、一个 channel、一个 style ID、一篇文章链接、一个素材网页和一份广告文案；国家、channel 与 style ID 下拉框支持模糊查询，国家支持多选，channel 与 style ID 可集中保存，已使用的 channel 会标记对应关键词，素材可直接打开，文案只提供一键复制而不在列表展示。列表支持自定义显示列，并可自由选择关键词、账户数量、实时指标、国家、channel、style ID、系列数量、预算或更新时间进行升序/降序排列，视图设置保存在当前浏览器。账户 ID 和 channel 不允许被不同关键词重复使用，搜索、分页及 Excel 导入导出同样支持这些规则。ClickFlare 收益由后台每 10 分钟同步到 MySQL，页面只查询数据库快照，不再等待上游接口。
...
MYSQL_CONTROL_CONNECTION_LIMIT=2
MYSQL_CONNECTION_TIMEOUT_MS=25000
MYSQL_QUERY_TIMEOUT_MS=20000
MYSQL_LOCK_TIMEOUT_SECONDS=10
MYSQL_LEAK_DETECTION_MS=30000
```

报表查询与登录鉴权使用独立连接池，避免同步任务或慢报表占满连接后无法登录。`MYSQL_CONNECTION_LIMIT` 是两个连接池共用的总连接预算，`MYSQL_CONTROL_CONNECTION_LIMIT` 表示其中为登录和用户权限预留的数量；例如总数为 5、预留 2 时，会分成 3 个报表连接和 2 个控制连接，不会额外突破数据库账号的连接上限。单条只读查询和数据库锁等待分别受 `MYSQL_QUERY_TIMEOUT_MS`、`MYSQL_LOCK_TIMEOUT_SECONDS` 限制，超时后会释放连接；连接占用超过 `MYSQL_LEAK_DETECTION_MS` 时会在服务日志中记录获取连接的位置，便于定位异常慢查询。
