...
人物风格的 AI 数据助手会根据状态切换肢体语言：待机时眨眼、点头和倾听，靠近时挥手、点赞，分析时托腮思考，回答完成后指向报表、双手展示并轻微庆祝。桌面端默认开启桌宠式自动漫游，人物会在页面下半区活动、根据方向转身，鼠标靠近、打开对话或分析数据时自动停下；标题栏的暂停/播放按钮可控制漫游。助手也支持拖动：关闭对话时拖动角色按钮，打开对话后拖动顶部标题栏。拖动位置会保存在当前浏览器，并自动限制在可视区域内。
