...
- `/jd`：京东 CPA 日报，提供优化师、日期、媒体、媒体账户和推客维度的汇总，以及优化师－日期下钻。京东预估/实际利润和 ROI 均包含“条件内预估赔付金额（当日）”，“有效订单数”口径为首购有效订单数与回流有效订单数之和。日期筛选区可选择是否排除未知优化师，该条件会作用于汇总卡片、全部分析维度、趋势图和下钻数据。
