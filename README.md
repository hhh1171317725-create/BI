...
- `/jd`：京东 CPA 日报，提供优化师、日期、媒体、媒体账户和推客维度的汇总，以及优化师－日期下钻。京东预估/实际利润和 ROI 均包含“条件内预估赔付金额（当日）”，“有效订单数”口径为首购有效订单数与回流有效订单数之和。可在页面展开“同步京东扣量比例”，粘贴回传策略列表 API 的完整 cURL；系统会全量查询 `configRatio`，按媒体账户 ID 关联并显示为百分比。日期筛选区默认排除未知优化师，也可切换为保留全部数据；该条件会作用于汇总卡片、全部分析维度、趋势图和下钻数据。
...
京东扣量 API 的请求地址和 Cookie 在成功同步后仅保存在服务器 `.runtime/jd-ratio-credentials.json`（权限 `600`），不会写入 Git。每日任务会尝试刷新扣量配置；京东登录态失效时继续保留上次成功数据，可在页面重新粘贴最新 cURL 更新。
