...
前端继续使用 `/`、`/jd`、`/tools` 和 `/login` 地址，并通过同源 `/api/*` 调用后端，因此页面功能和
...
- `/tools`：报表工具中心，集中提供报表查询预览、CSV 导出、每日消耗趋势 PNG 下载和数据字典下载。
...
4. 不要把整个网站代理到 Java。仅将 `/api/` 反向代理到
   `http://127.0.0.1:8765`，并为 `/login`、`/jd`、`/tools` 配置静态页面映射。可参考
