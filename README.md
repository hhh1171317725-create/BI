...
前端继续使用 `/`、`/jd`、`/jd-low-activity`、`/tools`、`/chat`、`/deeplink`、`/jd-images`、`/terminal`、`/account` 和 `/login` 地址，并通过同源 `/api/*` 调用后端，因此页面功能和
...
- `/tools`：工具中心，用于添加、打开和管理当前浏览器中的常用工具链接，并提供聊天室、京东深链、京东商品主图下载和管理员服务器终端入口。
- `/jd-images`：按 SKU 获取京东商品轮播主图，单次最多 20 个 SKU，图片和成功/失败明细统一打包为 ZIP。
...
4. 不要把整个网站代理到 Java。将 `/api/` 和支持 WebSocket 升级的 `/ws/` 反向代理到
   `http://127.0.0.1:8765`，并为 `/login`、`/jd`、`/jd-low-activity`、`/tools`、`/chat`、`/deeplink`、`/jd-images`、`/terminal`、`/account` 配置静态页面映射。可参考
