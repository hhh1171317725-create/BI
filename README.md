...
- `/account-vault`：管理员账户对应关系。每条记录保存一个关键词、多个账户 ID、多个投放国家、一个 channel、一个 style ID、一篇文章链接和一个素材网页；国家、channel 与 style ID 下拉框支持模糊查询，国家支持多选，channel 与 style ID 可集中保存，素材可从列表直接打开，并支持搜索、分页及 Excel 导入导出。
