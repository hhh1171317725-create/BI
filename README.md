...
- `/account-vault`：管理员账户对应关系。每条记录保存一个关键词、多个账户 ID、多个投放国家、一个 channel、一个 style ID、一篇文章链接、一个素材网页和一份广告文案；国家、channel 与 style ID 下拉框支持模糊查询，国家支持多选，channel 与 style ID 可集中保存，已使用的 channel 会标记对应关键词，素材可直接打开，文案只提供一键复制而不在列表展示。账户 ID 和 channel 不允许被不同关键词重复使用，搜索、分页及 Excel 导入导出同样支持这些规则。
