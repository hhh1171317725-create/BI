window.addEventListener('message', async event => {
  ...
  const {requestId,command,minutes,createdDays,clientUser,mainUserId}=event.data;
  if (typeof requestId!=='string' || !['status','start','stop','detect'].includes(command)) return;
  let result;
  try { result=await chrome.runtime.sendMessage({type:'bid-sync',command,minutes,createdDays,clientUser,mainUserId}); }
