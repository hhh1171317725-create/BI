...
## Server Scheduled Updates (2.0.0)
...
3. Choose the interval (5/10/15/30/60 minutes) and plan creation range (7/14/30/90 days). Click 保存并启用 and confirm encrypted storage. A blank Cookie preserves the saved value for that same upstream identity; changing either identity requires a fresh Cookie. Plaintext is cleared from the field after saving and is never stored in browser storage.
...
5. Each run reads Beijing's current-day statistics, stat_cost descending, at most two pages of 100 rows. The default creation range is the last 7 days inclusive. Older-created plans are excluded. Summary figures cover only the collected top 200, not all matching plans.
