...
## Browser Scheduled Updates (Extension 1.9.1)
...
2. In the same Chrome profile, log into Chuangliang and keep its page open. Log into BI and keep `/bid-monitor.html` open. Scheduled synchronization needs no copied Cookie. Click 读取当前创量用户 to fill `client-user` from the page's readable `userId` cookie (no session cookie is returned). The credentials area opens automatically; verify `main-user-id` for that same account. If userId is unreadable, the plugin reports that separately from a mismatched user; it does not infer that the login has expired. Detection itself does not enable synchronization.
