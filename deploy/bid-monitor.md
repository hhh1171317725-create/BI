...
## Browser Scheduled Updates (Extension 1.9.0)

1. Download `/downloads/ad-tools-helper.zip`, extract and reload the unpacked extension at `chrome://extensions`. Accept the added `alarms`, `storage` and `cl.mobgi.com` permissions. Refresh the BI page after updating.
2. In the same Chrome profile, log into Chuangliang and keep its page open. Log into BI and keep `/bid-monitor.html` open. Scheduled synchronization needs no copied Cookie. Expand the existing credential area to set `client-user` and `main-user-id` for that same Chuangliang account; the browser userId must match client-user.
...
5. The page checks plugin status every 20 seconds while visible. It refreshes displayed browser snapshots after a successful run, but does not overwrite a manually imported/query result. 读取最新快照 explicitly loads the latest saved result. Snapshot timestamps and date remain visible, including when they are old. Stop prevents subsequent runs; a save already in flight may complete.
