...
(() => {
  ...
  let rows=[],pageIndex=0,loaded=false,loading=false,asOf='',checkedAt='',unverified=0,checked=0,generation=0,timer=null,taskWarning='',loadError='';
  ...
  async function load(refresh=false){
    ...
    try{
      ...
      if(data.state==='running'){
        ...
        status.textContent=(data.progress||'后台正在核验累计消耗和转化…')+(elapsed!==null?` · 已用时 ${Math.floor(elapsed/60)}分${elapsed%60}秒`:'')+'。核验完成后自动显示结果。';
      ...
      const normalizedRows=data.rows.map(r=>{const normalized=B.normalize(r);return {...r,...normalized,search:[normalized.name,normalized.id,normalized.account,normalized.accountId,normalized.optimizer].join(' ').toLowerCase()};});
      status.textContent='累计数据核验完成，正在关联任务…';taskWarning='';let references=null;
      try{references=await window.loadBidStrategyReferences(data.asOf);}catch{taskWarning='日报任务读取失败，暂按已有任务配置匹配；可重新核验重试。';}
      if(current!==generation)return;
    ...
    }catch(error){if(current!==generation)return;loadError=error.message;draw();status.textContent=`核验失败：${error.message}。旧归档未用于本次预警，请重试。`;}
    if(current!==generation)return;
