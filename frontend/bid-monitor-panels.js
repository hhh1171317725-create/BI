...
(() => {
 ...
 const multiTools=make('div','multi-filter-tools'),multiSelectVisible=make('button','','全选当前'),multiClearVisible=make('button','','清除当前'),multiCount=make('span','multi-filter-count');
 ...
 function positionMulti(){if(multi.hidden||!multiButton)return;const rect=multiButton.getBoundingClientRect(),width=Math.min(400,window.innerWidth-24);multi.style.width=width+'px';multi.style.left=Math.max(12,Math.min(rect.left,window.innerWidth-width-12))+'px';multi.style.top=Math.max(12,Math.min(rect.bottom+6,window.innerHeight-multi.offsetHeight-12))+'px';}
 function refreshMulti(){
  const shown=[...multiList.children].filter(label=>!label.hidden).map(label=>label.querySelector('input'));
  const searching=multiSearch.value.trim();multiCount.textContent=`已选 ${multiList.querySelectorAll('input:checked').length} / ${multiList.children.length}${searching?` · 当前 ${shown.length}`:''}`;
