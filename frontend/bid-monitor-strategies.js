...
(() => {
  ...
  function drawDetail(){
    ...
    const metricHtml=metric('总消耗',number(total.cost),`${total.plans} 条计划`)+metric('预估赔付',number(total.estimatedCompensation))+metric('现金消耗',number(total.cashCost))+metric('转化数',number(total.conversions))+metric('注册数',number(total.registrations),'注册成本 '+number(total.cpa)+' 元')+metric('回传比例',percent(total.ratio))+metric('账户数',String(total.accounts))+metric('有消耗计划',String(total.spendingPlans||0))+(hasFinancial?metric('预估佣金',number(total.commission),`${total.priced} / ${total.plans} 条计划可计算`)+metric('预估 ROI',roi(total.estimatedRoi)):'');
    const financialColumns=hasFinancial?[['预估佣金','commission',number],['预估 ROI','estimatedRoi',roi]]:[],metricColumns=[['计划数','plans',String],['账户数','accounts',String],['总消耗','cost',number],['转化数','conversions',number],['注册数','registrations',number],['注册成本','cpa',number],['回传比例','ratio',percent],['预估赔付','estimatedCompensation',number],['现金消耗','cashCost',number],...financialColumns];
