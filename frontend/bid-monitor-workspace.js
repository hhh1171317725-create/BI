...
(() => {
  ...
  const levelOptions=[['plans','计划'],['accounts','账户'],['optimizers','优化师'],['tasks','任务'],['platforms','平台']];
  ...
  function enhanceSelection(){
    ...
    for(const row of bodyRows){const link=row.querySelector('.plan-detail-link');if(!link||row.querySelector('.ocean-select-cell'))continue;const id=row.querySelector('.plan-id-link')?.textContent.trim()||link.textContent.trim(),td=make('td','ocean-select-cell'),box=make('input');box.type='checkbox';box.className='ocean-row-select';box.checked=selectedRows.has(id);box.setAttribute('aria-label',`选择计划 ${link.textContent.trim()}`);td.append(box);row.prepend(td);box.onchange=()=>{if(box.checked)selectedRows.set(id,selectedSnapshot(row));else selectedRows.delete(id);updateBatch();};if(box.checked)selectedRows.set(id,selectedSnapshot(row));}
