...
function planCell(r,key){
  ...
  if(key==='task'){const inferred=r.taskSource==='inferred';const detail=r.inference||{};const title=inferred?`查询 ${detail.settlementPriceDate||'历史'} 日报：账户结算单价 ${fmt(detail.settlementPrice)}，匹配任务结算单价 ${fmt(detail.matchedPrice)}`:'';return `<td${title?` title="${esc(title)}"`:''}>${esc((r.task||names[r.pricingStatus]||'未匹配')+(inferred?'（反推）':''))}</td>`;}
