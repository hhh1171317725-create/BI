...
let raw=[],analyzed=[],visible=[],optimizerRows=[],taskRules=[],page=1,range=null,source='',busy=false,followSync=true,abort;
...
function render(){
  ...
  const optimizerMode=$('#viewMode').value==='optimizers';
  let key=$('#sort').value;if(optimizerMode&&key==='bid'){key='cost';$('#sort').value='cost';}
  ...
  optimizerRows=B.aggregateOptimizers(visible).sort((a,b)=>(b[key]??-Infinity)-(a[key]??-Infinity));
  const displayRows=optimizerMode?optimizerRows:visible,size=Number($('#pageSize').value),pages=Math.max(1,Math.ceil(displayRows.length/size));page=Math.min(page,pages);
  if(optimizerMode){
    $('#tableHead').innerHTML='<tr><th>优化师</th><th>计划数</th><th>账户数</th><th>价格匹配</th><th>总消耗</th><th>转化数</th><th>注册数</th><th>回传比例</th><th>佣金</th><th>预估赔付</th><th>现金消耗</th><th>现金利润</th><th>预估 ROI</th><th>出价利润率</th></tr>';
    $('#rows').innerHTML=displayRows.slice((page-1)*size,page*size).map(r=>`<tr><td>${esc(r.optimizer)}</td><td>${r.plans}</td><td>${r.accounts}</td><td>${r.priced} / ${r.plans}</td><td>${fmt(r.cost)}</td><td>${fmt(r.conversions)}</td><td>${fmt(r.registrations)}</td><td>${r.ratio===null?'--':fmt(r.ratio*100)+'%'}</td><td>${fmt(r.commission)}</td><td>${fmt(r.estimatedCompensation)}</td><td>${fmt(r.cashCost)}</td><td class="${r.profit===null?'':r.profit<0?'bad':'good'}">${fmt(r.profit)}</td><td>${fmtRoi(r.estimatedRoi)}</td><td class="${r.bidProfitRate===null?'':r.bidProfitRate<0?'bad':'good'}">${fmtPercent(r.bidProfitRate)}</td></tr>`).join('')||'<tr><td colspan="14" class="empty">没有符合条件的优化师</td></tr>';
  }else{
    $('#tableHead').innerHTML='<tr><th>计划</th><th>账户</th><th>优化师</th><th>任务</th><th>结算单价</th><th>总消耗</th><th>转化数</th><th>注册数</th><th>回传比例</th><th>当前出价</th><th>预估 ROI</th><th>盈亏线出价</th><th>出价利润率</th></tr>';
  ...
  $('#count').textContent=optimizerMode?`${displayRows.length} 名优化师（${visible.length} 条计划）`:`${displayRows.length} 条`;$('#pageLabel').textContent=`第 ${page} / ${pages} 页`;$('#prev').disabled=page<=1;$('#next').disabled=page>=pages;$('#export').disabled=!displayRows.length;
}
$('#fetch').onclick=async()=>{
...
for(const id of ['search','viewMode','taskFilter','filter','sort','pageSize'])$('#'+id).addEventListener('input',()=>{page=1;render();});
...
$('#export').onclick=()=>{
  ...
  const optimizerMode=$('#viewMode').value==='optimizers';
  const rows=optimizerMode?[['优化师','计划数','账户数','已匹配价格计划数','统计开始','统计结束','总消耗','转化数','注册数','回传比例','佣金','预估赔付金额','现金消耗','现金利润','预估ROI','出价利润率'],
    ...optimizerRows.map(r=>[r.optimizer,r.plans,r.accounts,r.priced,range.start,range.end,r.cost,r.conversions,r.registrations,r.ratio,r.commission,r.estimatedCompensation,r.cashCost,r.profit,fmtRoi(r.estimatedRoi),fmtPercent(r.bidProfitRate)])]:[['计划ID','计划名称','账户ID','账户名称','优化师','任务','结算单价','统计开始','统计结束','总消耗','转化数','注册数','回传比例','当前出价','佣金','预估赔付金额','预估ROI','规则赠款','现金消耗','盈亏线出价','出价利润率'],
  ...
  const url=URL.createObjectURL(new Blob(['\ufeff'+rows.map(row=>row.map(cell).join(',')).join('\r\n')],{type:'text/csv;charset=utf-8'}));const a=document.createElement('a');a.href=url;a.download=`出价监测_${optimizerMode?'优化师汇总':'计划明细'}_${range.start}_${range.end}.csv`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);
};
