...
function planCell(r,key){
  ...
  if(key==='task'){const linked=r.taskSource==='inferred'||r.taskSource==='daily-report',detail=r.inference||{};const title=r.taskSource==='daily-report'?`查询 ${detail.taskDate||'历史'} 大航海日报：账户任务名 ${detail.reportedTaskName||'--'}`:r.taskSource==='inferred'?`查询 ${detail.settlementPriceDate||'历史'} 日报：账户结算单价 ${fmt(detail.settlementPrice)}，匹配任务结算单价 ${fmt(detail.matchedPrice)}`:'';const suffix=r.taskSource==='daily-report'?'（日报）':r.taskSource==='inferred'?'（反推）':'';return `<td${title?` title="${esc(title)}"`:''}>${esc((r.task||names[r.pricingStatus]||'未匹配')+(linked?suffix:''))}</td>`;}
