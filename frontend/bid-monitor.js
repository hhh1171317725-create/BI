...
function planCell(r,key,index){
  ...
  if(key==='name'){
    const destination=B.platformAdLink(r),id=esc(r.id);
    const idLink=destination?.exact?`<a class="plan-id-link" href="${esc(destination.url)}" target="_blank" rel="noopener noreferrer" title="在创量打开这条广点通广告">${id}</a>`:`<button type="button" class="plan-id-link" data-plan-detail="${index}" title="查看这条广告的数据与计算依据">${id}</button>`;
    const platformLink=destination&&!destination.exact?` <a class="plan-source-link" href="${esc(destination.url)}" target="_blank" rel="noopener noreferrer" title="打开创量字节广告列表，需按广告 ID 搜索">创量列表↗</a>`:'';
    return `<td><button type="button" class="plan-detail-link" data-plan-detail="${index}" title="查看计划数据与计算依据">${esc(r.name||'未命名计划')}</button><small>${idLink}${platformLink}</small>${compensationWarningsReady()&&r.compensationShortfall>0?`<span class="compensation-badge" title="计划累计消耗已超过当前出价×7.2，累计转化不足6个；当前预估赔付仍为0">还差 ${r.compensationShortfall} 个转化</span>`:''}</td>`;
  }
