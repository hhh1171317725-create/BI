...
function planCell(r,key){
  ...
  if(key==='task'){const inferred=r.taskSource==='inferred';const detail=r.inference||{};const title=inferred?`按账户汇总反推：出价 ${fmt(detail.weightedBid)} × 回传比例 ${fmtPercent(detail.callbackRatio)}，推算结算单价 ${fmt(detail.impliedSettlementUnit)}，任务结算单价 ${fmt(detail.expectedSettlementUnit)}`:'';return `<td${title?` title="${esc(title)}"`:''}>${esc((r.task||names[r.pricingStatus]||'未匹配')+(inferred?'（反推）':''))}</td>`;}
