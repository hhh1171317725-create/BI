...
window.analyzeBidStrategyRows=(rows,references=null)=>{
  ...
  rows=B.withOverallConversions(rows);const inferred=B.inferAccountTasks(rows,taskRules,references.accounts||{},references);
  return rows.map(row=>B.analyzeTask(row,taskRules,0,20,false,null,inferred.get(B.inferenceIdentity(row))));
};
...
function planCell(r,key,index){
  ...
  if(key==='task'){const detail=r.inference||{};const title=r.taskSource==='daily-report'?`查询 ${detail.taskDate||'历史'} 大航海日报：账户任务名 ${detail.reportedTaskName||'--'}`:r.taskSource==='open-url'?`按 open_url 判断任务：${{'same-daily-url':'与已确认日报任务的计划使用同一链接','same-push-id':'与已确认日报任务的链接具有相同 outPushPlanId','configured-url':'匹配任务配置的 URL 特征','task-name-in-url':'链接包含唯一的任务名称'}[detail.urlMatch]||'URL 唯一匹配'}`:!r.task&&r.openUrl?'open_url 未能唯一对应任务；可设置 URL 特征或等待同链接计划的日报数据':r.missingReason||'';const suffix=r.taskSource==='daily-report'?'（日报）':r.taskSource==='open-url'?'（open_url）':'';return `<td${title?` title="${esc(title)}"`:''}>${esc((r.task||names[r.pricingStatus]||'未匹配')+suffix)}</td>`;}
...
function drawValueFilters(){
  ...
  const optimizer=$('#optimizerFilter'),chosenOptimizers=new Set([...optimizer.selectedOptions].map(option=>option.value));
  const optimizers=[...new Set(analyzed.map(row=>row.optimizer||'未填写'))].sort((a,b)=>a.localeCompare(b,'zh-CN'));
...
$('#export').onclick=()=>{
  ...
  let rows=aggregateMode?[[...dimensions.map(d=>dimensionLabels[d]),'统计开始','统计结束',...exportMetrics.map(([label,key])=>exportLabel[key]||label)],
    ...
    ...visible.map(r=>[r.id,r.name,r.platform,r.accountId,r.account,r.optimizer,r.task,r.taskSource==='daily-report'?'大航海日报任务名':r.taskSource==='open-url'?'open_url 识别':r.task?'账户名匹配':'',r.basePrice,priceSourceLabel(r),range.start,range.end,r.cost,r.conversions,r.overallConversions,r.registrations,r.ratio,r.bid,r.commission,r.estimatedCompensation,fmtRoi(r.estimatedRoi),r.grant,r.cashCost,r.breakEvenBid,fmtPercent(r.bidProfitRate),r.gap,r.price,...visibleOptional.map(column=>r[column[1]])])];
