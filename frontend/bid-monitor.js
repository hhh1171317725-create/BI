...
function planCell(r,key,index){
  ...
  const title=key==='gap'?gapTitle(r.accountId,r):key==='basePrice'?(priceSource||r.priceReason||r.missingReason):key==='price'?(priceSource?`${priceSource}单价 × ${r.gapSource==='task-reference'?'同任务参考 gap':'账户 gap'}`:r.missingReason):key==='ecpm'?(r.ecpm===null?'曝光数、转化数或当前出价缺失，无法估算':'当前出价 × 转化数 ÷ 曝光数 × 1000'):key==='estimatedRoi'?(r[key]===null?r.missingReason:`预估赔付金额：${fmt(r.estimatedCompensation)}`):key==='bidProfitRate'?(r[key]===null?r.missingReason:`盈亏线出价：${fmt(r.breakEvenBid)}`):r[key]===null?r.missingReason:'';
