...
(async()=>{
 ...
 await new Promise(resolve=>server.listen(0,'127.0.0.1',resolve));
  ...
  assert.equal(await page.locator('#bidMultiFilterDropdown .multi-filter-head strong').textContent(),'选择任务');assert.equal(await page.locator('#bidMultiFilterDropdown .multi-filter-list').evaluate(el=>getComputedStyle(el).flexDirection),'column');assert.equal(await page.locator('#bidMultiFilterDropdown .column-choice').first().evaluate(el=>el.getBoundingClientRect().width/el.parentElement.getBoundingClientRect().width>.9),true);await page.locator('#bidMultiFilterDropdown').screenshot({path:path.resolve(__dirname,'../.runtime/bid-multi-filter-desktop.png')});
  ...
  await page.evaluate(()=>{window.filterRenderCount=0;document.addEventListener('bid:rendered',()=>window.filterRenderCount++);});
  await multi.getByRole('button',{name:'全选当前',exact:true}).click();
  assert.equal(await page.locator('#optimizerFilter').evaluate(select=>select.selectedOptions.length),0);
  await multi.getByRole('searchbox').fill('李四');
  await multi.getByRole('button',{name:'清除当前',exact:true}).click();
  assert.equal(await page.locator('#count').textContent(),'105 条');
  assert.equal(await page.evaluate(()=>window.filterRenderCount),0,'Draft selection must not rebuild the report');
  await multi.locator('.multi-filter-done').click();
  assert.equal(await page.evaluate(()=>window.filterRenderCount),1,'Apply multiple selections in one render');
  ...
  assert.equal(await multi.getByRole('button',{name:'全选当前',exact:true}).isDisabled(),true);
  await multi.getByRole('searchbox').fill('李四');
  await multi.getByRole('button',{name:'全选当前',exact:true}).click();
  assert.equal(await page.locator('#count').textContent(),'53 条');
  ...
  await selectView('tasks');assert.match(await page.locator('#count').textContent(),/^2 个任务（105 条计划）$/);
  ...
  await selectView('optimizerTasks');assert.match(await page.locator('#count').textContent(),/^2 个优化师 × 任务组合（105 条计划）$/);
  ...
  await selectView('conversionTargets');assert.match(await page.locator('#count').textContent(),/^2 个目标组合（105 条计划）$/);
  ...
  await page.locator('#accountDrillBack').click();await selectView('plans');
  ...
  await selectView('optimizers');assert.match(await page.locator('#count').textContent(),/1 条计划/);
  await selectView('plans');
  ...
  await page.locator('#reportRefresh').click();
  await page.waitForFunction(()=>!document.querySelector('#reportRefresh').disabled&&document.querySelector('#historyStatus').textContent.includes('2 / 2 个数据日期'));
  assert.equal(await page.evaluate(()=>historyMode),true,'Refresh must retain the historical report');
  assert.equal(await page.locator('#historyStart').inputValue(),'2026-09-01');
  assert.equal(await page.locator('#historyEnd').inputValue(),'2026-09-02');
  assert.match(await page.locator('#count').textContent(),/^2 天（4 条计划）$/);
  await selectView('dateAccounts');
  ...
  await selectView('plans');await page.locator('#search').fill('history-1');assert.equal(await page.locator('#rows tr').count(),1);assert.equal(await page.locator('#rows tr td').nth(6).textContent(),'125.00');
  ...
  assert.equal(await page.locator('#rows').textContent().then(text=>text.includes('ready-1')),false);
  ...
  await page.waitForFunction(()=>document.querySelector('#endedWarningsDialog tbody').textContent.includes('过期计划 0'));
  assert.match(await page.locator('#endedWarningsDialog [role=status]').textContent(),/当前不是完整结果/);
  assert.equal(await page.locator('#endedWarningsDialog .ended-reload').isDisabled(),true);
  assert.equal(await page.locator('#endedWarningsDialog input[type=search]').first().isEnabled(),true);
  ...
  assert.match(await page.locator('#endedWarningsDialog tbody').textContent(),/还差 1 个/);assert.match(await page.locator('#endedWarningsDialog tbody').textContent(),/出价 10.00 × 7.2/);assert.match(await page.locator('#endedWarningsDialog tbody').textContent(),/超过门槛 28.00/);
  ...
  assert.equal(await page.locator('#endedWarningsDialog tbody tr').count(),26);assert.match(await page.locator('#endedWarningsDialog tbody').textContent(),/创建于 2026-09-12/);
  await page.locator('#endedWarningsDialog .ended-task-filter summary').click();
  await page.locator('#endedWarningsDialog .ended-task-panel input[type=search]').fill('任务乙');
  await page.locator('#endedWarningsDialog .ended-task-options input[value="任务乙"]').check();
  assert.match(await page.locator('#endedWarningsDialog [role=status]').textContent(),/当前匹配 0 个/);
  await page.locator('#endedWarningsDialog .ended-task-panel input[type=search]').fill('');await page.locator('#endedWarningsDialog .ended-task-options input[value="任务甲"]').check();
  assert.equal(await page.locator('#endedWarningsDialog tbody tr').count(),26);assert.equal(await page.locator('#endedWarningsDialog .ended-task-filter summary').textContent(),'任务（2）');
  await page.keyboard.press('Escape');assert.equal(await page.locator('#endedWarningsDialog').isVisible(),true);
  await page.locator('#endedCreatedStart').fill('2026-09-14');assert.match(await page.locator('#endedWarningsDialog [role=status]').textContent(),/开始日期不能晚于/);
  await page.locator('#endedWarningsDialog .ended-clear').click();assert.equal(await page.locator('#endedWarningsDialog tbody tr').count(),50);
  await page.setViewportSize({width:390,height:844});await page.locator('#endedWarningsDialog .ended-task-filter summary').click();
  const taskPanelBounds=await page.locator('#endedWarningsDialog .ended-task-panel').boundingBox();assert.ok(taskPanelBounds.x>=0&&taskPanelBounds.x+taskPanelBounds.width<=390);
  await page.screenshot({path:path.resolve(__dirname,'../.runtime/bid-ended-filters-mobile.png')});
  await page.keyboard.press('Escape');await page.locator('#endedWarningsDialog .ended-close').click();assert.equal(await page.locator('#count').textContent(),countBefore);
