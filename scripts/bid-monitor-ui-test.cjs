...
(async()=>{
 const server=http.createServer((req,res)=>{const file=path.join(root,path.basename(req.url));if(!fs.existsSync(file)){res.writeHead(404);res.end();return}res.setHeader('Content-Type',file.endsWith('.js')?'application/javascript':'text/html;charset=utf-8');res.end(fs.readFileSync(file))});
 ...
 try{
  ...
  await page.route('**/api/**',route=>{const url=route.request().url();let data={};if(url.endsWith('/session'))data={authenticated:true};else if(url.endsWith('/tool-visibility'))data={bidMonitor:true};else if(url.endsWith('/import'))data={rows:sample};route.fulfill({json:data})});
  ...
  assert.deepEqual(errors,[]);console.log('UI PASS: import, formulas, search, pagination, export, mobile width, no script errors');
