...
test('conversion warning uses strict cost boundary and overall conversions without granting compensation',()=>{
 ...
 const prepared=B.withOverallConversions([{...base,cost:23,id:'1',accountId:'a',platform:'字节',conversions:1}],[{id:'1',accountId:'a',platform:'字节',cost:50,conversions:4},{id:'1',accountId:'b',platform:'字节',cost:1000,conversions:100}]);
 assert.equal(prepared[0].overallCost,73);
...
test('retains optional Chuangliang fields for report columns and filters',()=>{
 const result=normalize({platform_text:'广点通',show_cnt:'20000',convert_cnt:'15',cpa_bid:'25',app_type_text:'小程序',deep_bid_type_text:'深度转化',deep_cpabid:'88.5',deep_external_action_text:'深度付费',external_action_text:'注册',status_text:'投放中'});
...
test('aggregates every plan by optimizer with weighted metrics',()=>{
 ...
 assert.equal(result[0].cost,750);assert.equal(result[0].conversions,16);assert.equal(result[0].registrations,30);assert.equal(result[0].profit,140);
...
test('cash ROI deducts grant only when both strict conditions hold',()=>{
 ...
 assert.equal(cashMetrics({...base,conversions:5},10).grant,0);
...
test('estimated ROI adds compensation when conversion cost exceeds 1.2 times bid',()=>{
 ...
 assert.equal(sixConversions.grant,540);assert.equal(sixConversions.estimatedCompensation,540);assert.equal(sixConversions.estimatedRoi,640/600);
 for(const conversions of [0,1,5])assert.equal(cashMetrics({...base,conversions},10).estimatedCompensation,0);
...
test('cash summary applies grant per plan and weights ROI and bid profit by their denominators',()=>{
 ...
 assert.equal(total.roi,300/160);assert.equal(total.bidProfitRate,140/300);
