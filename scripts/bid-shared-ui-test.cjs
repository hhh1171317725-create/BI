...
(async()=>{
 ...
 try{
  ...
  await page.route('**/api/**',route=>{
   ...
   else if(url.pathname==='/api/bid-monitor/history/ended-warnings')data={state:'ready',asOf:'2026-09-17',checkedAt:'2026-09-17T02:00:00Z',checkedCount:0,rows:[]};
  ...
  await page.locator('#openEndedWarnings').click();await page.waitForFunction(()=>document.querySelector('#endedWarningsDialog [role=status]').textContent.includes('共 0 个预警计划'));
