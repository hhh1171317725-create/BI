...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.tool:visible').count(), 8);
    assert.equal(await page.locator('[data-tool="terminal"]').count(), 0);
    await page.getByRole('link',{name:'出价监测',exact:true}).waitFor();
    assert.equal(await page.locator('[data-tool="bidMonitor"]').count(),0);
    ...
    assert.equal(await page.locator('.tool:visible h3').textContent(),'Todo 任务');
    await page.reload();await page.getByRole('button',{name:'取消收藏：Todo 任务',exact:true}).waitFor();
    await page.locator('#favoriteToolsOnly').click();assert.equal(await page.locator('.tool:visible').count(),1);
    await page.locator('#resetToolFilters').click();assert.equal(await page.locator('.tool:visible').count(),8);
    await page.getByRole('button',{name:'取消收藏：Todo 任务',exact:true}).click();

    await page.locator('#toolSearch').fill('出价');
    assert.equal(await page.locator('.tool:visible').count(), 0);
    assert.equal(await page.getByRole('link',{name:'出价监测',exact:true}).isVisible(),true);
    ...
    await page.waitForFunction(()=>document.getElementById('toolResultCount').textContent==='显示 2 / 8 项工具');
    ...
    await page.reload();await page.waitForFunction(()=>document.getElementById('toolResultCount')?.textContent==='显示 0 / 0 项工具');
    assert.equal(await page.locator('.tool:visible').count(),0);
    assert.equal(await page.getByRole('link',{name:'出价监测',exact:true}).isVisible(),true);
    ...
    await page.waitForFunction(()=>document.querySelector('a[href="/bid-monitor.html"]')?.classList.contains('report-visibility-hidden'));
