...
class BidEndedWarningServiceTest {
  ...
  @Test void backfilledSixConversionsRemoveOldWarningAndUnavailableRowsAreNotAssumedZero()throws Exception{
    ...
    when(sync.collectPlanTotals(anyMap(),eq("private-cookie"),anyList(),eq(today),any())).thenReturn(List.of(row("1",6),row("2",5)));
    ...
    service.read(7,false);verify(sync,times(1)).collectPlanTotals(anyMap(),anyString(),anyList(),any(),any());
  }
  @Test void concurrentViewersShareJobAndErrorsDoNotReturnOldRowsOrSecrets()throws Exception{
    ...
    when(sync.collectPlanTotals(anyMap(),anyString(),anyList(),any(),any())).thenAnswer(call->{entered.countDown();assertTrue(release.await(3,TimeUnit.SECONDS));throw new IllegalArgumentException("private-cookie");});
    ...
    verify(sync,times(1)).collectPlanTotals(anyMap(),anyString(),anyList(),any(),any());
  }
  @Test void verificationLongerThanCacheTtlStillDeliversResultAndRetainsItForFiveMinutes()throws Exception{
    when(history.readEndedPlans(7,today)).thenReturn(List.of(row("1",3)));
    when(sync.collectPlanTotals(anyMap(),anyString(),anyList(),any(),any())).thenAnswer(call->{
    ...
    verify(sync,times(2)).collectPlanTotals(anyMap(),anyString(),anyList(),any(),any());
  }
