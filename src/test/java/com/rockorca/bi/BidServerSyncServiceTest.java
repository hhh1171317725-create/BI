...
class BidServerSyncServiceTest {
  ...
  private List<Map<String,Object>> rows(int offset,int size){
    ...
    for(int i=0;i<size;i++){
      ...
      row.put("show_cnt",20000);row.put("cpm_platform",18.75);row.put("app_type_text","小程序");row.put("deep_bid_type_text","深度转化");row.put("deep_cpabid",88.5);
  ...
  @Test void readsAllFourPagesAndKeepsIdsAndMetrics()throws Exception{
    ...
    assertEquals(20000,row.get("show_cnt"));assertEquals(18.75,row.get("cpm_platform"));assertEquals("小程序",row.get("app_type_text"));assertEquals("深度转化",row.get("deep_bid_type_text"));
