...
class GdtBidMonitorClientTest {
  ...
  @Test void mapsGdtFieldsIntoTheSharedBidMonitorSchema(){
    ...
    raw.put("cost","123.45");raw.put("conversions_count","12");raw.put("deep_conversions_count","34");raw.put("bid_amount","5.5");
    ...
    assertEquals("123.45",row.get("stat_cost"));assertEquals("12",row.get("convert_cnt"));assertEquals("34",row.get("active_register"));
    assertEquals("5.5",row.get("cpa_bid"));assertEquals("账户甲",row.get("media_account_name"));assertEquals("优化师甲",row.get("user_name"));
